...
CREATE TABLE figure (
  figure_number INTEGER NOT NULL PRIMARY KEY,
  ...
  exclusive_id INTEGER NOT NULL references exclusive(exclusive_id));

  
  Create Index IX_figurefigure
  on figure(figure_id);

  Create Index IX_figurebrand
  on figure(brand_id);
  
  create index IX_figurecategory
  on figure(category_id);
  
  create index IX_figureexclusive
  on figure(exclusive_id);
